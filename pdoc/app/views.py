...
def video_calling(request,slug):
    ...
    if minutes < 20 or now > appointment.time:
        return render(request,"videocalling.html",{"doctor":doctor,"name":name,"practice_id":practice_id,"designation":designation,"customer":customer, 'slug':slug})
...
def save_prescription_firebase(filename,customer):
    if request.method == 'POST':
        ...
        prescription_folder_name = 'records/{}/{}'.format(customer, name)

        if not os.path.exists(prescription_folder_name):
            os.mkdir(prescription_folder_name)
        
        filename.save(os.path.join(settings.MEDIA_ROOT, prescription_folder_name, name))
        
        return HttpResponse('Prescription Saved Successfully')

    else:
        return HttpResponse('Prescription Save Failed')
    # name = filename.split("/")[-1]
